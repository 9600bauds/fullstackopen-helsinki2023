...
const App = () => {
  ...
  function getDetailsFromError(error) {
    if (error.response && error.response.data.error) {
      ...
      const errorMessages = Object.values(errorDetails).map(
        (err) => err.message
      );
  ...
  const addEntry = (event) => {
    ...
    PersonService.create(newEntry)
      ...
      .catch((error) => {
        console.log(error);
        showErrorMessage(
          `Failed to create new person! Error: ` + getDetailsFromError(error),
          5000
        );
      });
  };
    ...
    PersonService.update(oldEntry.id, newEntry)
      .then((response) => {
        const newState = persons.map((p) =>
          p.id !== newEntry.id ? p : response
        );
        setPersons(newState);
        showSuccessMessage(
          `Successfully updated ${newEntry.name} to a new number.`,
          5000
        );
      })
      .catch((error) => {
        console.log(error);
        showErrorMessage(
          `Failed to update person! Error: ` + getDetailsFromError(error),
          5000
        );
      });
  };
  ...
  const deleteHook = (id) => {
    ...
    PersonService.deleteByID(id)
      ...
      .catch((error) => {
        showErrorMessage(
          `Could not delete ${person.name}! Error: ${getDetailsFromError(
            error
          )}`,
          5000
        );
  ...
  useEffect(() => {
    PersonService.getAll()
      ...
      .catch((error) => {
        showErrorMessage(
          `Couldn't load server data! Error: ${getDetailsFromError(error)}`,
          5000
        );
      });
